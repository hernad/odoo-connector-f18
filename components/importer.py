...
class F18Importer(Component):
    ...
    _name = "f18.importer"
    _inherit = ["base.importer", "f17.base"]
    ...
    def _is_uptodate(self, binding):
        ...
        # precision).
        if binding.f18_updated_at:
            return external_date < binding.f17_updated_at
        return False
    ...
    def _create(self, data):
        ...
        _logger.debug("%s created from F18 %s", binding, self.external_id)
        return binding
    ...
    def _update(self, binding, data):
        ...
        if not data:
            _logger.debug(
                "%s not updated from F18 %s as nothing changed",
                binding,
                self.external_id,
            )
            return
        ...
        _logger.debug("%s updated from F18 %s", binding, self.external_id)
        return
    ...
    def _handle_record_missing_on_external_source(self):
        """Hook called when we are importing a record missing on external source
        ...
        if binding:
            ...
            binding.unlink()
        return _("Record does no longer exist in external source")
    ...
    def run(self, external_id, force=False, record=None, **kwargs):
        ...
        :param external_id: identifier of the record on external source
        """
        ...
        if record is not None:
            ...
            try:
                ...
                return self._handle_record_missing_on_external_source()
        binding = self._get_binding()
        if not binding:
...
class F18Deleter(Component):
    _name = "f18.deleter"
    _inherit = ["base.deleter", "f18.base"]
