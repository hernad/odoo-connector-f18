...
class F18ProjectBaseFields(models.AbstractModel):
    """F18 Project Base fields

    Shared by the binding f18.project.project
    and the wizard to link/create a F18 project
    """

    _name = "f18.project.base.mixin"
    _description = "F18 Project Base Mixin"
...
class F18ProjectProject(models.Model):
    _name = "f18.project.project"
    _inherit = ["jira.binding", "jira.project.base.mixin"]
    _inherits = {"project.project": "odoo_id"}
    _description = "F18 Projects"
        ...
        for binding in self:
            ...
            if len(export_bindings) > 1:
                raise exceptions.ValidationError(
                    _(
                        "Only one F18 binding can be configured with the Sync."
    ...
    def write(self, values):
        ...
        self._ensure_f18_key()
        return res

    def _ensure_f18_key(self):
...
class ProjectProject(models.Model):
    ...
    f18_key = fields.Char(
        string="F18 Key",
        compute="_compute_f18_key",
        store=True,
    )

    @api.depends("f18_bind_ids.f18_key")
    def _compute_f18_key(self):
        for project in self:
            keys = project.mapped("f18_bind_ids.f18_key")
            project.f18_key = ", ".join(keys)
