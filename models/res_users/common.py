...
class UserAdapter(Component):
    _name = "f18.res.users.adapter"
    _inherit = ["f18.dbase.adapter"]
...
class F18ResUsers(models.Model):
    _name = "f18.res.users"
    _inherit = "f18.binding"
    _inherits = {"res.users": "odoo_id"}
    _description = "F18 User"
...
class ResUsers(models.Model):
    ...
    f18_bind_ids = fields.One2many(
        comodel_name="f18.res.users",
    ...
    def button_link_with_f18(self):
        self.ensure_one()
        self.link_with_external_source(raise_if_mismatch=True)
        if not self.f18_bind_ids:
            raise exceptions.UserError(_("No F18 user could be found"))

    def link_with_external_source(self, backends=None, raise_if_mismatch=False):
        if backends is None:
            backends = self.env["f18.backend"].search([])
        result = {}
        for backend in backends:
            ...
            with backend.work_on("f18.res.users") as work:
                ...
                for user in self:
                    ...
                    if existing:
                        ...
                        continue
                    try:
                        binding = self.env["f18.res.users"].create(
